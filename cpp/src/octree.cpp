...
bool OcTree::IsNotNew(std::shared_ptr<Node3> n)
{
  ...
  if (northWestFront->IsNotNew(n))
    return true;
  if (northEastFront->IsNotNew(n))
    return true;
  if (southWestFront->IsNotNew(n))
    return true;
  if (southEastFront->IsNotNew(n))
    return true;
  if (northWestBack->IsNotNew(n))
    return true;
  if (northEastBack->IsNotNew(n))
    return true;
  if (southWestBack->IsNotNew(n))
    return true;
  if (southEastBack->IsNotNew(n))
    return true;
...
bool OcTree::Remove(std::shared_ptr<Node3> n, std::unordered_set<OcTree *> &octs)
{
  ...
  if (!res)
    res |= northEastFront->Remove(n, octs);
  if (!res)
    res |= southWestFront->Remove(n, octs);
  if (!res)
    res |= southEastFront->Remove(n, octs);
  if (!res)
    res |= northWestBack->Remove(n, octs);
  if (!res)
    res |= northEastBack->Remove(n, octs);
  if (!res)
    res |= southWestBack->Remove(n, octs);
  if (!res)
    res |= southEastBack->Remove(n, octs);
...
bool OcTree::Update(std::shared_ptr<Node3> n)
{
  ...
  if (northWestFront->Update(n))
    return true;
  if (northEastFront->Update(n))
    return true;
  if (southWestFront->Update(n))
    return true;
  if (southEastFront->Update(n))
    return true;

  if (northWestBack->Update(n))
    return true;
  if (northEastBack->Update(n))
    return true;
  if (southWestBack->Update(n))
    return true;
  if (southEastBack->Update(n))
    return true;
...
{
  std::cout << "in: " << boundary.getHalfLength() << std::endl;
  std::cout << "insertAABB: " << boundary.intersectsAABB(range) << std::endl;

  // Automatically abort if the range does not intersect this quad
  if (!boundary.intersectsAABB(range) || IsEmptyLeaf())
  {
    std::cout << "QueryNonEmptyLevelC0: " << boundary.getHalfLength() << std::endl;
    return; // empty list
  }

  if (IsLeaf())
  { // no children
    std::cout << "QueryNonEmptyLevelC1: " << boundary.getHalfLength() << std::endl;
    if (boundary.getHalfLength() > OcTree::param.cluster_halfleng + 0.001)
    {
      return;
    }
  ...
  std::cout << "QueryNonEmptyLevelC: " << boundary.getHalfLength() << std::endl;
  if (boundary.getHalfLength() > OcTree::param.cluster_halfleng + 0.001)
  {
    // Otherwise, add the points from the children
