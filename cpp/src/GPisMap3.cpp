                           ...
                           float *res)
{
  ...
  for (int i = start_idx; i < end_idx; ++i)
  {
    ...
    else if (sqdst.size() > 1)
    {
      ...
      std::size_t n(0);
      std::generate(std::begin(idx), std::end(idx), [&]
                    { return n++; });
      std::sort(std::begin(idx), std::end(idx), [&](int i1, int i2)
                { return sqdst[i1] < sqdst[i2]; });
